...
function addAfinger () {
    ...
    for property in "${properties[@]}"; do
        ...
        if [[ ! $property =~ ^[A-Za-z0-9_]{1,}=([A-Za-z0-9_]{1,},?){1,} ]]; then
            dispError "3" "Incorrect format for property : ${OR}$property${NC}" && return 1
        fi
        # Split name & value
        ...
        local action="create"
        local existing_property=$(getVar "./usrs/$username/profile" "info_$prop_name")
        [[ ! -z $existing_property ]] && action="replace"
        # Array of values ?
        [[ $prop_values == *","* ]] && prop_values="(${prop_values//,/)(})"
        # Set new property
        setVar "info_$prop_name" "./usrs/$username/profile" "$action" "$prop_values"
    done
...
function removeAfinger () {
    ...
    for property_name in "${properties_names[@]}"; do
        local existing_property=$(getVar "./usrs/$username/profile" "info_$property_name")
        [[ -z $existing_property ]] && dispError "3" "The property \"$property_name\" doesn't exists" && return 1
    done
    # Remove properties
    for property_name in "${properties_names[@]}"; do
        setVar "info_$property_name" "./usrs/$username/profile" "remove"
    done
